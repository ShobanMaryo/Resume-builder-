...
export default function App() {
  ...
  const [font, setFont] = useState("font-sans"); // Default Sans
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row font-sans">
      ...
      <div className="w-full md:w-1/3 p-6 overflow-y-auto h-screen border-r bg-gray-50">
        ...
        <div className="mb-6 bg-white p-4 rounded-lg shadow-sm">
          ...
          <div className="flex gap-4">
            {/* Template Switcher */}
            <button onClick={() => setTemplate(template === 'classic' ? 'modern' : 'classic')} className="flex flex-col items-center gap-1">
               <LayoutTemplate size={20} className="text-gray-600" />
               <span className="text-xs">{template === 'classic' ? 'Classic' : 'Modern'}</span>
            </button>
            {/* Color Picker */}
            <div className="flex flex-col items-center gap-1">
             ...
             {/* Font Switcher */}
             <button onClick={() => setFont(font === 'font-sans' ? 'font-serif' : 'font-sans')} className="flex flex-col items-center gap-1">
               <Type size={20} className="text-gray-600" />
               <span className="text-xs">{font === 'font-sans' ? 'Sans' : 'Serif'}</span>
            </button>
          </div>
        </div>
        ...
        <FormSection title="Experience">
          {data.experience.map((exp) => (
            <div key={exp.id} className="mb-4 pb-4 border-b last:border-0 relative">
              <button onClick={() => removeItem("experience", exp.id)} className="absolute top-0 right-0 text-red-500 hover:text-red-700">
                <Trash2 size={16} />
              </button>
          ...
          <button
            ...
            className="w-full py-2 flex items-center justify-center gap-2 border-2 border-dashed border-gray-300 rounded text-gray-500 hover:border-blue-500 hover:text-blue-500"
          >
        ...
        <FormSection title="Skills">
            <div className="flex flex-wrap gap-2">
                {data.skills.map((skill, index) => (
                    <input 
                        key={index}
                        className="p-2 border rounded w-1/3 flex-grow"
                        value={skill}
      ...
      <div className="w-full md:w-2/3 bg-gray-200 p-8 overflow-y-auto h-screen flex justify-center">
        ...
        <div 
          ref={componentRef} 
          className={`bg-white shadow-2xl w-[210mm] min-h-[297mm] p-10 ${font} text-gray-800`}
        >
          {/* Header */}
          <header className={`pb-6 mb-6 ${template === 'modern' ? 'flex justify-between items-start border-b-2' : 'text-center border-b'}`} style={{ borderColor: color }}>
            <div>
                <h1 className="text-4xl font-bold uppercase tracking-wide" style={{ color: color }}>{data.personal.fullName}</h1>
                <p className="text-lg mt-2 text-gray-600">{data.personal.summary}</p>
            </div>
            <div className={`mt-4 text-sm text-gray-500 ${template === 'modern' ? 'text-right' : 'mt-2'}`}>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            {/* Experience Section */}
            <section>
              <h2 className="text-xl font-bold uppercase mb-4 pb-1 border-b" style={{ borderColor: color, color: color }}>Experience</h2>
              {data.experience.map((exp) => (
                <div key={exp.id} className="mb-4">
                  ...
                  <p className="text-sm mt-1 text-gray-600 whitespace-pre-line">{exp.description}</p>
                </div>
              ))}
            </section>
            ...
            {/* Education Section */}
            <section>
              <h2 className="text-xl font-bold uppercase mb-4 pb-1 border-b" style={{ borderColor: color, color: color }}>Education</h2>
            ...
            {/* Skills Section */}
            <section>
              <h2 className="text-xl font-bold uppercase mb-4 pb-1 border-b" style={{ borderColor: color, color: color }}>Skills</h2>
              <div className="flex flex-wrap gap-2">
                {data.skills.map((skill, index) => (
